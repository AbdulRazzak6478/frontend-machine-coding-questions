...
useEffect(() => {
  ...
  async function fetchProducts(){
    ...
    // const data = await axios(`https://dummyjson.com/products?limit=10?skip=${page * 10 - 10}`);
  ...
  return (
    <>
     <div className="products-container">
      ...
      {
        products.length > 0 && products.slice(page*10 - 10, page*10).map((product,index)=>{
     ...
     <div className="pagination-container">
      <div className="pagination">
        ...
        <button className={`next ${page < products.length / 10 ? '': 'next-disabled'}`} onClick={()=>selectPageHandler(page+1)}>next</button>
      </div>
     </div>
     }
    </>
